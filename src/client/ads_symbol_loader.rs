...
fn parse_datatype_entry_item(item : &AdsDatatypeEntry, buffer: &Box<[u8]>, offset : usize ) -> Option<AdsDataTypeInfo> {
    ...
    if let Ok(nm) = String::from_utf8(buffer[name_start ..name_end].to_vec()) {
        ...
        if ret.num_array_dimensions > 0 {
            ...
            let array_info_len : usize = std::mem::size_of::<AdsDatatypeArrayInfo>().try_into().unwrap();

            let mut arr_size = 1;
            for _i in 0 .. ret.num_array_dimensions {

                let array_info_end= array_info_offset + array_info_len;                    
                let arr_info_item:&AdsDatatypeArrayInfo = unsafe{ &*buffer[array_info_offset..array_info_end].as_ptr().cast() };
...
pub fn upload_symbols( ams_address : &AmsAddr, port : i32) -> Option<AdsSymbolCollection> {
    ...
    unsafe {
        let read_len_err = AdsSyncReadReqEx2( 
            ...
            0x0,
            std::mem::size_of::<AdsSymbolUploadInfo2>().try_into().unwrap(),
            ptr_upload_info,
            ptr_bytes_read
        );
    
        if read_len_err != 0 {
            log::error!("Failed to upload SymbolUploadInfo2 with error {}", read_len_err);
            return None;
        }    
    }


    log::info!("The controller contains {} symbols with a total size of {} and data type size of {}",
    ...
    unsafe {
        ...
        if symbol_upload_err != 0 {
            log::error!("Failed to upload symbol information with error {}", symbol_upload_err );
            return None;
        }
    ...
    let mut uploaded_symbols:Vec<AdsSymbolInfo> = Vec::with_capacity(upload_info.nSymbols as usize);
    ...
    for _i in 0 .. upload_info.nSymbols {
        ...
        let name_bytes = buffer[symbol_name_offset..symbol_name_offset + item.nameLength as usize].to_vec();
        let type_bytes = buffer[type_name_offset..type_name_offset + item.typeLength as usize].to_vec();
        let comment_bytes = buffer[comment_offset..comment_offset + item.commentLength as usize].to_vec();
